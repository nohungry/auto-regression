"""
KS 視覺健康度驗證（DOM metrics，非截圖比對）— Super9娛樂城
KS-VIS-001~006

與 tests/rc/feature/visual/test_visual.py 意圖對齊，參考 tests/lt 的 modal 型變體。

KS 站差異：
- 框架 Nuxt (Vue)，金色英文主題；登入為 modal（非 route）：
  首頁 → dismiss_announcement（單層公告）→ open_login_modal（button.gold-btn）
- input 皆 .input-style，靠 type 屬性區分帳密
- 登入指標為 nav wallet 圖示（.nav-bg img[alt='Wallet']，登入後才渲染）；無 .balance-color
...
@pytest.mark.visual
class TestVisual:
    """KS-VIS-001~006：視覺健康度驗證（版面、破圖、對齊）"""
    ...
    def test_home_no_horizontal_overflow(self, page: Page, site_config):
        """KS-VIS-001：首頁沒有明顯橫向超框（scrollWidth <= innerWidth + 4）"""
    ...
    def test_home_no_broken_images(self, page: Page, site_config):
        """KS-VIS-002：首頁圖片資源沒有明顯破圖（complete 且 naturalWidth=0）
    ...
    def test_home_text_not_clipped(self, page: Page, site_config):
        """KS-VIS-003：首頁主要文案未明顯被裁切（允許 ellipsis 設計，排除 overflow:hidden 節點）"""
        self._open_home_page(page, site_config)
        sh = get_screenshotter(page)
    ...
    def test_login_modal_no_horizontal_overflow(self, page: Page, site_config):
        """KS-VIS-004：登入 modal 開啟後頁面沒有明顯橫向超框"""
    ...
    def test_login_form_alignment(self, page: Page, site_config):
        """KS-VIS-005：登入 modal 帳號/密碼輸入框左右對齊（x 誤差 ≤ 2px，寬度誤差 ≤ 30px）
