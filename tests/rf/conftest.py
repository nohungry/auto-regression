"""
rf 站點測試專用 conftest（金爺娛樂城／base-modal 彈窗 .base-modal__container）
- 覆寫 site_config：tests/rf/ 下不需加 --site=rf。
...
def go_home(class_logged_in_page, site_config):
    """[RF 覆寫] 每個測試前回首頁並清進站彈窗。"""
    reset_home_with_home_popups(class_logged_in_page, site_config.url, "rf")
    yield
