"""
LU 視覺健康度驗證（DOM metrics，非截圖比對）— Dlgbet
LU-VIS-001~006

與 tests/rc/feature/visual/test_visual.py 意圖對齊，參考 tests/lt 的 modal 型變體。

LU 站差異：
- 框架 Nuxt (Vue)，登入為 modal（非 route）：首頁 → dismiss_announcement（雙層彈窗：
  圖片廣告 → 文字公告）→ open_login_modal
- 登入 modal 容器 .dialog-container.max-w-[708px]，input 無 .input-style，用 type 區分帳密
- LU 不顯示 username，登入指標改用 nav 餘額 span（.fixed.top-0.z-50 span.text-xs.font-bold.text-white）
- 破圖檢查沿用 LT 防禦：排除 src='' 空圖（僅抓真·載入失敗）

fixture 決策（LU-VIS-006）：
  LU 的 feature 測試一律用 class_logged_in_page + go_home（smoke 才自行登入），
  站內無使用 logged_in_page fixture 的先例。為與 LU feature 慣例一致，本條登入態
  測試改用 class_logged_in_page + go_home（其餘四站沿用 rc 模板的 logged_in_page）。
"""
...
@pytest.mark.visual
class TestVisual:
    """LU-VIS-001~006：視覺健康度驗證（版面、破圖、對齊）"""
    ...
    def _reach_login_form(self, page: Page, site_config) -> LoginPage:
        """開首頁 → 關雙層彈窗 → 開登入 modal（等帳號 input 可見）。"""
    ...
    def test_home_no_horizontal_overflow(self, page: Page, site_config):
        """LU-VIS-001：首頁沒有明顯橫向超框（scrollWidth <= innerWidth + 4）"""
    ...
    def test_home_no_broken_images(self, page: Page, site_config):
        """LU-VIS-002：首頁圖片資源沒有明顯破圖（complete 且 naturalWidth=0）
    ...
    def test_home_text_not_clipped(self, page: Page, site_config):
        """LU-VIS-003：首頁主要文案未明顯被裁切（允許 ellipsis 設計，排除 overflow:hidden 節點）"""
        self._open_home_page(page, site_config)
        sh = get_screenshotter(page)
    ...
    def test_login_modal_no_horizontal_overflow(self, page: Page, site_config):
        """LU-VIS-004：登入 modal 開啟後頁面沒有明顯橫向超框"""
    ...
    def test_login_form_alignment(self, page: Page, site_config):
        """LU-VIS-005：登入 modal 帳號/密碼輸入框左右對齊（x 誤差 ≤ 2px，寬度誤差 ≤ 30px）
    ...
    def test_home_avatar_or_login_in_viewport(self, class_logged_in_page: Page, go_home, site_config):
        """LU-VIS-006：登入後 nav 登入指標（餘額 span）在視窗內（未超出左右/上邊界）

        fixture：LU feature 慣例用 class_logged_in_page + go_home（站內無 logged_in_page
        先例），與其餘四站的 logged_in_page 不同。
        """
