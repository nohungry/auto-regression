"""
RF 後台側欄入口檢測（menu entry detection）— 站長 + 代理兩層級

驗證「側欄有哪些入口 + 各入口對應的子入口」與預期 spec 完全一致（含順序）：
- 頂層入口：父項 a.memberSpan 的 route id（'' = 非路由項目，如修改密碼/登出）
- 子入口：父項底下葉節點的 href（collapsed DOM 即存在，毋須展開）
選單增刪、順序、權限變動都會 fail —— 後台改版 / 權限異動的第一訊號。
RF 與 rc/re/lt/rd 同構但選單較精簡（features/visit-control 子項較少、代理報表無
bet-report），故 spec 獨立維護。

斷言策略：結構性識別（route id / href），不綁文案；文案僅在 spec 註解供人類對照。
spec 來源：2026-07-30 實機 probe（headless chromium 直連 dev 站）。

⚠️ 帳號層級（RF fixture 命名與信用版四站相反）：dashboard_page = 站長、
   agent_dashboard_page = 代理；用 go_dashboard / go_agent_dashboard 確保落點穩定。
"""
...
class TestAgentMenuEntries:
    """RF-DASH-MENU-001：代理側欄入口 + 子入口集合與 spec 精確一致。"""

    def test_agent_menu_tree_matches_spec(self, go_agent_dashboard, site_config):
        """dump 側欄選單樹（route id + 子入口 href）→ 與代理層級 spec 全等比對。"""
        Mgmt = get_dashboard_management_page_class(site_config.site_id)
        tree = Mgmt(go_agent_dashboard).menu_tree()
...
class TestMasterMenuEntries:
    """RF-DASH-MENU-002：站長側欄入口 + 子入口集合與 spec 精確一致。"""

    def test_master_menu_tree_matches_spec(self, go_dashboard, site_config):
        """dump 側欄選單樹 → 與站長層級 spec 全等比對。"""
        Mgmt = get_dashboard_management_page_class(site_config.site_id)
        tree = Mgmt(go_dashboard).menu_tree()
