...
⚠️ 帳號層級：**站長帳號**（SITE_QW_DASHBOARD_USER）。QW dashboard POM re-export LU，
   主錢包充值流程與 LU 同構（probe 2026-06-25：會員 dqwauto01 Main wallet 欄 index 5、
   點金額開「Main wallet」彈窗）。
...
目標會員用 site_config.username（前台測試帳號 dqwauto01，站長底下穩定會員）。
"""
...
class TestMasterMainWalletTopup:
    ...
    def test_main_wallet_adjust_reversible(self, dashboard_page: Page, site_config):
        ...
        target = site_config.username  # SITE_QW_USERNAME（= dqwauto01），不寫死帳號
