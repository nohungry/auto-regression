...
登入帳號來自 .env：
- SITE_RD_DASHBOARD_AGENT_USER/PASS（自動化代理 drdautotes）
- 代理走「無 -admin」代理入口 dashboard_agent_url（canonical：-admin=站長 / 無-admin=代理）
- 代理 2FA：傳 dashboard_agent_totp（RD 代理目前無，條件式自動跳過）
"""
...
def dashboard_page(browser, site_config):
    ...
    使用 .env 的 SITE_RD_DASHBOARD_AGENT_USER/PASS（自動化代理）。
    ...
    login.goto_and_login(
        ...
        site_config.dashboard_agent_totp,  # 代理 2FA（RD 代理目前無；條件式自動跳過）
    )
