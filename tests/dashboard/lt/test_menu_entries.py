"""
LT 後台側欄入口檢測（menu entry detection）— 代理 + 總代/站長兩層級
...
⚠️ 帳號層級：dashboard_page = 代理（SITE_LT_DASHBOARD_AGENT_USER）、
   master_dashboard_page = 總代/站長（SITE_LT_DASHBOARD_USER）；權限不同 → spec 分開。
"""
...
class TestAgentMenuEntries:
    """LT-DASH-MENU-001：代理側欄入口 + 子入口集合與 spec 精確一致。"""
...
class TestMasterMenuEntries:
    """LT-DASH-MENU-002：總代/站長側欄入口 + 子入口集合與 spec 精確一致。"""
