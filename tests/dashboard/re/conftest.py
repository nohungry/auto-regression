"""
RE 後台測試專用 conftest（BeWin，信用版 /management 後台）
- dashboard_page：代理 SITE_RE_DASHBOARD_AGENT_USER/PASS（限管理/報表權限，無 2FA）
- master_dashboard_page：總代/站長 SITE_RE_DASHBOARD_USER/PASS（供總代→代理派點，無 2FA）
...
def dashboard_page(browser, site_config):
    ...
    yield from dashboard_login_session(
        ...
        totp=site_config.dashboard_agent_totp,  # 代理 2FA（RE 目前無，條件式跳過）
    )
