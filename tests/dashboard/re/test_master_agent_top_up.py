"""
RE 後台 — 總代（站長層級）對下線代理派點測試（存入/提取）
RE-DASH-MASTER-TOPUP-001

信用版三層額度模型：總代 → 代理 → 會員，上層派點 = 上層扣、下層加。
既有 test_member_top_up 測「代理→會員」這層；本檔補測**最上層「總代→代理」**。

⚠️ 帳號層級：**總代帳號**（SITE_RE_DASHBOARD_USER，無 2FA）。
...
目標代理用 site_config.dashboard_agent_user（= re 代理，總代直屬下線、與 member 測試同帳號）。
"""
...
class TestMasterToAgentTopUp:
    """RE-DASH-MASTER-TOPUP-001：總代對代理存入/提取額度驗證（只驗代理側，總代 ∞）。"""
    ...
    def test_master_deposit_withdraw_to_agent(self, master_dashboard_page: Page, site_config):
        ...
        agent = site_config.dashboard_agent_user  # SITE_RE_DASHBOARD_AGENT_USER，不寫死帳號
