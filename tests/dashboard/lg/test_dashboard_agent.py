"""
LG 後台**代理層級** read-only smoke（login 無 2FA + 側欄 + 導航 + 登出）

⚠️ 帳號層級：本檔以代理 SITE_LG_DASHBOARD_AGENT_USER 登入，入口為**無 -admin** 的
   SITE_LG_DASHBOARD_AGENT_URL。LG 代理後台與 LU 同為 Vue admin 結構，page object
   re-export LU。
...
存提（deposit/withdraw）不在本檔：LG 代理為空帳號（0 會員），無法做對稱 balance 驗證，
待有測資的代理帳號再補。
...
class TestAgentDashboardLogin:
    """LG-DASH-AGENT-001：代理登入（無 2FA）打通驗證"""
...
class TestAgentDashboardNavigation:
    """LG-DASH-AGENT-002：代理側欄 + 導航 read-only smoke"""
