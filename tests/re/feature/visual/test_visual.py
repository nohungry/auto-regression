"""
RE 視覺健康度驗證 (BeWin) — DOM metrics，非截圖比對
與 tests/rc/feature/visual/test_visual.py 意圖對齊，selector 完全沿用（共用平台）。
"""
...
class TestVisual:
    ...
    def test_home_no_horizontal_overflow(self, page: Page, site_config):
        ...
        if sh: sh.full_page(f"verify_首頁橫向溢出檢測_scroll{metrics['scrollWidth']}_inner{metrics['innerWidth']}")
    ...
    def test_home_no_broken_images(self, page: Page, site_config):
        ...
        """)
        sh = get_screenshotter(page)
        if sh: sh.full_page(f"verify_首頁破圖檢測_broken{len(broken)}")
    ...
    def test_home_text_not_clipped(self, page: Page, site_config):
        ...
        sh = get_screenshotter(page)
        if sh: sh.full_page(f"verify_首頁文案裁切檢測_overflow{len(overflow_nodes)}")
    ...
    def test_login_modal_no_horizontal_overflow(self, page: Page, site_config):
        ...
        sh = get_screenshotter(page)
        if sh: sh.full_page(f"verify_登入Modal橫向溢出檢測_scroll{metrics['scrollWidth']}_inner{metrics['innerWidth']}")
        assert metrics["scrollWidth"] <= metrics["innerWidth"] + 4, \
    ...
    def test_login_form_alignment(self, page: Page, site_config):
        ...
        sh = get_screenshotter(page)
        if sh: sh.full_page(f"verify_登入表單對齊檢測_{metrics}")
    ...
    def test_home_avatar_or_login_in_viewport(self, logged_in_page: Page, site_config):
        ...
        sh = get_screenshotter(page)
        if sh: sh.full_page(f"verify_avatar視窗範圍檢測_{metrics}")
