"""
RE 首頁公告大圖輪播彈窗 功能測試 (BeWin)
RE-TC-F01 ~ RE-TC-F04

[2026-04-29 實機驗證] RE 站首頁**未實作**自動彈出的公告大圖輪播彈窗
（`.popup-announcement-mask` 不存在於 DOM）。整類測試以 `pytest.skip`
標記為「未實作」，待產品確認是否要加上此功能後再啟用。
"""
...
pytestmark = pytest.mark.skip(reason="RE 站首頁未實作公告大圖輪播彈窗（待產品確認）")
...
class TestAnnouncementPopup:
    """
    RE-TC-F01 ~ RE-TC-F04：首頁公告大圖輪播彈窗行為
    ...
    def test_popup_close_btn_advances_slide(self, page: Page, site_config):
        """RE-TC-F02：點擊 ✕ 後輪播推進下一張（彈窗仍存在，未直接關閉）"""
        page.goto(site_config.url)
        page.wait_for_load_state("networkidle")
        mask = page.locator(".popup-announcement-mask")
        expect(mask).to_be_visible(timeout=5000)
    ...
    def test_popup_dont_show_today(self, page: Page, site_config):
        """RE-TC-F04：勾選「今天不再顯示」後，重新進入首頁不再出現彈窗（待確認行為）"""
        pass
