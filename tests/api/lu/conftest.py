...
@pytest.fixture(scope="session")
def api_base_url():
    url = os.getenv("SITE_LU_API_URL")
    if not url:
        raise ValueError("請在 .env 設定 SITE_LU_API_URL")
    return url.rstrip("/")
...
def api_headers(site_config):
    ...
    """
    domain = os.getenv("SITE_LU_API_DOMAIN")
    if not domain:
        raise ValueError("請在 .env 設定 SITE_LU_API_DOMAIN")
    companycode = os.getenv("SITE_LU_COMPANYCODE", "dlu")
