...
class TestMemberAPI:
    ...
    def test_get_member_info_returns_account(self, site_config, api_base_url, auth_headers):
        """TC-API-RF-MEM-001: 已認證時取會員資料應回傳 status=Success 且 account 相符"""
    ...
    def test_get_member_info_without_token_fails(self, api_base_url, api_headers):
        """TC-API-RF-MEM-002: 無 authorization 時應無法取得會員資料

        Probe 確認：無 token 回 HTTP 401，status=Error。
        """
    ...
    def test_get_member_info_with_invalid_token_fails(self, api_base_url, api_headers):
        """TC-API-RF-MEM-003: 無效 token 時應無法取得會員資料

        Probe 確認：無效 token 回 HTTP 401，status=Error，errorCode=PermissionDenied。
        """
