...
API server 與 LT 共用同一個 API endpoint（見 .env SITE_RC_API_URL），靠 header `companycode` / `domain` 區分站點。
RC 的 companycode=rc，domain 見 .env SITE_RC_API_DOMAIN。

Login endpoint 與 response 結構與 LT 一致：
    POST /api/Member/memberLogin → {status: "Success", data: {token}}

執行方式：
    .venv/bin/pytest tests/api/rc/test_auth.py -v
...
class TestAuthAPI:
    ...
    def test_login_returns_token(self, site_config, api_base_url, api_headers):
        """TC-API-RC-001: 正確帳密應回傳 200 且 data.token 為非空字串"""
    ...
    def test_login_wrong_password_returns_error(self, site_config, api_base_url, api_headers):
        """TC-API-RC-002: 錯誤密碼應回傳非 200 或 status != Success"""
        session = requests.Session()
        resp = session.post(
            api_base_url + LOGIN_PATH,
            json={
