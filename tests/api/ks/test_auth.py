"""
KS API 認證測試（Super9娛樂城）

API server 與 LT 共用同一套 t9platform API endpoint（見 .env SITE_KS_API_URL），
靠 header `companycode` / `domain` 區分站點。RE 的 companycode=dre，domain 見
.env SITE_KS_API_DOMAIN。
...
執行方式：
    .venv/bin/pytest tests/api/ks/test_auth.py -v
...
class TestAuthAPI:
    ...
    def test_login_returns_token(self, site_config, api_base_url, api_headers):
        """TC-API-KS-001: 正確帳密應回傳 200 且 data.token 為非空字串"""
    ...
    def test_login_wrong_password_returns_error(self, site_config, api_base_url, api_headers):
        """TC-API-KS-002: 錯誤密碼應回傳非 200 或 status != Success"""
        session = requests.Session()
        resp = session.post(
            api_base_url + LOGIN_PATH,
            json={
    ...
    def test_logout_returns_success(self, site_config, api_base_url, api_headers):
        """TC-API-KS-003: 登出 API 回傳 Success
