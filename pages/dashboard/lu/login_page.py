...
2. 點 Login → 彈出 Two-Factor Authentication modal（`.dialog-container`）
...
class DashboardLoginPage:
    ...
    def _fill_totp(self, totp_secret: str):
        """等 2FA modal → 逐格填 6 碼 → Confirm"""
        sh = get_screenshotter(self.page)

        # 等 Two-Factor Authentication modal 出現
        self.otp_dialog.first.wait_for(state="visible", timeout=10000)
