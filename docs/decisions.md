...
## D-026 現金版後台 2FA 登入:>=35s 間隔節流 + Verify 結果判定 + 上限一次重送

- 狀態:accepted(2026-08-28 nohungry 拍板)
- 決策:現金版(LU/LG/QW,共用 `pages/dashboard/lu/login_page.py`)後台 2FA 登入加三層處理。① **頻率節流**:`utils/dashboard_helpers._throttle_2fa_login()`,module-level 常數 `TWOFA_LOGIN_MIN_INTERVAL_S = 35.0`(規則 2b「>30s」+ 裕度;>30s 亦保證跨 TOTP 窗口),狀態 `_last_2fa_login_at` 用 `time.monotonic()`、**刻意不持久化到磁碟**(跨 process 節流要 lock / 陳舊值失效,複雜度遠高於收益;CI 各站分 job、本機守 D-011 同帳號不並行,單 session 內節流已覆蓋實際場景)。gate 放在 `dashboard_login_session()` 建 context **之前**、條件為 `totp is not _OMIT and totp`(不硬編站點清單,新站零維護;今日僅命中 LG 站長 / LU 站長+代理 / QW 站長+代理共 5 個登入,信用版 TOTP 皆空故零行為改變)。時間戳在**檢查點**就更新,使失敗的 attempt 也計入間隔。② **Verify 結果判定**:`_submit_totp_code()` 以 `expect_response("/TwoFactorAuth/Verify")` 包住 Confirm 點擊(`VERIFY_RESPONSE_TIMEOUT = 15000`);攔不到 → 不重送、交還 `verify_login_success` 原路徑;`resp.ok` → 正常返回;非 ok → `get_next_window_totp_code()` 跨窗口取新碼、清空 6 格、重送**恰好一次**;第二發仍非 ok → 全頁截圖 `2FA_Verify_二次失敗` + `RuntimeError`(訊息含兩次 HTTP status、第二次 body 前 200 字與冷卻 20-30 分鐘提示),**絕不第三次**。③ **失敗可觀測**:`verify_login_success()` 逾時先 `sh.full_page("verify_後台登入失敗")` 再拋。兩處 `time.sleep`(節流、等窗口翻轉)為 **D-006 核可例外**,援引該條「LU 因真守衛硬等 + 2FA 風險刻意保留」的既有框架。條件式 2FA 的前兩段分流(4s 偵測 modal、無 secret 即返回)維持原樣,不破壞 D-017 語意。
- 理由:2026-08-28 實測釐清 `POST /api/TwoFactorAuth/Verify` 回 `400 {"error":{"code":"AuthenticationFailed"}}` 的真因。**QW 站長與代理的 TOTP secret 都是正確的**(各自成功登入過,使用者手動登入亦成功),先前「secret 疑遭伺服器端重綁」的歸因不成立;「OTP 填滿自動送出」假說也被推翻(填滿 6 格零 Verify 請求,必須點 Confirm 才送)。真因是頻率與競態兩面向:QW 代理→站長兩次 2FA 登入間隔僅數秒,遠低於規則 2b 的 30s;高負載下「產碼 → 填 6 格 → 送出」可能超過 `get_totp_code` 的 5 秒緩衝。規則 2b 自 2026-06 就寫明對策,但只存在於文件、無任何程式強制,因此每次都靠人記得 —— 這次把它變成程式碼層的硬約束。同時 2FA 失敗完全不可觀測(點完 Confirm 不看結果、失敗不截圖),表現只是後續空等 `.sidebar-nav` 到逾時,每次都要重跑才能猜原因;判 Verify HTTP 結果讓失敗訊息直接指名成因與處置。重送上限設 1 且必須跨窗口:同窗口重放後端必拒(等於白送一次還加深頻率計數),而無上限重試正是規則 2b 明載的鎖定成因。
- 替代方案:① **重用 session / storage_state 跳過重複登入**(不採:session-scoped fixture 已保證每個帳號整場只登入一次,QW 那兩次 2FA 是**不同帳號**(站長 vs 代理)故重用零收益;且後台 token 存放位置未 probe,若在 sessionStorage 則 `storage_state` 根本涵蓋不到,要先做 probe 才知可行性,成本高於本方案而收益不明)。② 只加節流不判 Verify(不採:節流降低發生率但無法歸零——碼過期競態與後端狀態不受間隔控制,且失敗仍零證據)。③ 只判 Verify 不加節流(不採:等於每次都靠重送救,反而增加登入次數,與規則 2b 相悖)。④ 用 `--reruns` / `@pytest.mark.flaky` 重試整條測試(不採:pytest.ini 明文立場,且整條重試會重登入,正是 lockout 成因)。⑤ 節流間隔設 31s 貼齊下限(不採:裕度太薄,產碼與網路往返即可吃掉)。⑥ 節流狀態寫檔跨 process 共享(不採,如上)。⑦ 失敗改 skip / xfail(不採:D-009,且登入類缺陷本就不該被 gate 掉)。
- 影響:`utils/totp_helper.py`(新增 `get_next_window_totp_code`)、`utils/dashboard_helpers.py`(節流常數 / 狀態 / `_throttle_2fa_login` + `dashboard_login_session` gate)、`pages/dashboard/lu/login_page.py`(**LU/LG/QW 三站共用**,D-020 式 re-export,改動三站同時生效)、`docs/dashboard-technical-notes.md` 規則 2b、`CLAUDE.md` 後台段與 utils 區、`README.md` utils 清單、`tests/dashboard/{lu,qw}/test_menu_entries.py` 檔頭歸因。信用版 rc/re/lt/rd/rf 不受影響(不帶 TOTP,不進 gate、不走本 POM)。
