...
> 維護方式：產品修好一項就移除該列並 un-gate 對應測試。最後更新：2026-07-21。
...
| 8 | **RC** | **登入 API 成功但 SPA 不轉場**：`/api/Member/memberLogin` 回 200 Success（含 token），前端停在 /login、表單保留、不進首頁 | 儀器化 probe 2026-07-21：API 200 + token 確認，15s 後仍 /login。WSL CDP 環境當日 4 連發重現；**CI headless 未重現**（同日 4 輪 CI rc smoke 全綠）、同 session `test_login_success` 亦可過 → 前端登入回應處理存在 timing 敏感 race | `test_home_page_loads`（logged_in_page fixture 會 ERROR；**未加 xfail 守門**因 CI 綠、僅本機重現）；測試側已加送出 retry + 表單關閉守衛（PR #152），站點修復後自動轉綠 | 中（登入主流程間歇壞） |
